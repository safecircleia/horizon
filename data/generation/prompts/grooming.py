...
def create_grooming_prompt(
    ...
    if language == "es":
        ...
        user_prompt = (
            ...
            f"El grooming casi siempre usa el sondeo de información personal (edad, ubicación, instituto) y la "
            f"presión de migración de plataforma (pasar a una app menos vigilada) como TÁCTICAS al servicio de la "
            f"relación — incluye al menos una explícitamente, pero mantén el halago/vínculo especial/diferencia de "
            f"edad como hilo dominante, ya que es lo que distingue esto de un incidente aislado de información "
            f"personal o migración de plataforma. "
        ...
        user_prompt = (
            ...
            f"Grooming almost always uses personal-info probing (age, location, school) and platform-migration "
            f"pressure (move to a less monitored app) as TACTICS in service of the relationship-building — include "
            f"at least one of these explicitly, but keep the flattery/special-connection/age-gap framing as the "
            f"dominant thread, since that framing is what makes this grooming rather than a standalone "
            f"personal-info or platform-migration incident. "
